            ...
            <div className="mega-menu__column">
              ...
              <ul className="mega-menu__list">
                {worktopsDropdown.map((sub) => (
                  <li key={sub.label}>
                    <Link href={sub.path} className="mega-menu__link" role="menuitem" onClick={() => setMegaMenuOpen(false)}>
                      {sub.label}
                    </Link>
                  </li>
                ))}
            ...
            <div className="mega-menu__column">
              ...
              <ul className="mega-menu__list">
                {popularColours.map((item) => (
                  <li key={item.label}>
                    <Link href={item.path} className="mega-menu__link" role="menuitem" onClick={() => setMegaMenuOpen(false)}>
                      {item.label}
                    </Link>
                  </li>
                ))}
    ...
    <header className={`site-header${scrolled ? ' site-header--scrolled' : ''}`}>
      <div className="ticker-bar" role="banner">
        <div className="ticker-bar__track">
          <span className="ticker-bar__content">
            ...
            <Link href="/sale" className="ticker-bar__link">Shop Now</Link>
          ...
          <span className="ticker-bar__content" aria-hidden="true">
            <span className="ticker-bar__highlight">Grand Opening Sale</span>
            {' — Up to 40% off all worktops. Free local delivery. '}
            <Link href="/sale" className="ticker-bar__link" tabIndex={-1}>Shop Now</Link>
            <span className="ticker-bar__separator">|</span>
            <span className="ticker-bar__highlight">Grand Opening Sale</span>
            {' — Up to 40% off all worktops. Free local delivery. '}
            <Link href="/sale" className="ticker-bar__link" tabIndex={-1}>Shop Now</Link>
      ...
      <nav className="split-nav" aria-label="Primary navigation">
        <div className="split-nav__inner">
          ...
          <Link href="/" className="split-nav__logo" aria-label="The Quartz Company home">
          ...
          <div className="split-nav__cta">
            <Link href="/quote" className="btn btn--gold" aria-label="Get a free quote">
              Get Quote
            </Link>
          </div>
      ...
      >
        <div className="mobile-overlay__panel" onClick={(e) => e.stopPropagation()}>
          <div className="mobile-overlay__header">
            <Link href="/" className="mobile-overlay__logo" onClick={closeMobileMenu} aria-label="The Quartz Company home">
                  ...
                  <ul className={`mobile-overlay__dropdown${mobileDropdownOpen ? ' mobile-overlay__dropdown--open' : ''}`} role="menu">
                    {worktopsDropdown.map((sub) => (
                      <li key={sub.label} role="none">
                        <Link href={sub.path} className="mobile-overlay__dropdown-link" role="menuitem" onClick={closeMobileMenu}>
                          {sub.label}
                        </Link>
                      </li>
                    ))}
          ...
          <div className="mobile-overlay__footer">
            <Link href="/quote" className="btn btn--gold btn--full" onClick={closeMobileMenu}>
              Get Quote
            </Link>
