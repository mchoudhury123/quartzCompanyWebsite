...
export default function TaskCard({ label, count, icon: Icon, color, to, href, badge, highlight }) {
  const inner = (
    <div className="task-card" style={{ '--card-bg': color || '#c5a47e' }}>
      ...
      <div className={`task-card__count${highlight ? ' task-card__count--highlight' : ''}`}>{count ?? ''}</div>
